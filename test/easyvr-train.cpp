 ...
 *
 * train          train (r0) (r1)...            train 0 2 45               Train
 *records
 * load           load (r0) (r1) ...            load 0 51 2 3              Load
 *records
 * clear          clear                         clear
 *remove all records in  Recognizer
 * record         record / record (r0) (r1)...  record / record 0 79       Check
 *record train status
 * vr             vr                            vr                         Check
 *recognizer status
 * getsig         getsig (r)                    getsig 0                   Get
 *signature of record (r)
 * sigtrain       sigtrain (r) (sig)            sigtrain 0 ZERO            Train
 *one record(r) with signature(sig)
 * settings       settings                      settings                   Check
...
int cmdRecord(int len, int paraNum) {
  ...
  if (paraNum == 1) {
    ...
    ret = myVR.checkRecord(buf, records,
                           paraNum - 1); // auto clean duplicate records
    printSeperator();
    if (ret >= 0) {
 ...
 * buf[i+1]   -->  Record number.(0xFF: Not loaded(Nongroup mode), or not set
 * (Group mode)) (i= 0, 1, ... 6)
...
const unsigned int io_pw_tab[16] = {10, 15, 20,  25,  30,  35,  40,  45,
...
void printHelp(void) {
  ...
  printSeperator();
  //  Serial.println(F("--------------------------------------------------------------------------------------------------------------"));
  Serial.println(F("train          train (r0) (r1)...            train 0 2 45  "
                   "             Train records"));
  Serial.println(F("load           load (r0) (r1) ...            load 0 51 2 3 "
                   "             Load records"));
  Serial.println(F("clear          clear                         clear         "
                   "             remove all records in  Recognizer"));
  Serial.println(F("record         record / record (r0) (r1)...  record / "
                   "record 0 79       Check record train status"));
  Serial.println(F("vr             vr                            vr            "
                   "             Check recognizer status"));
  Serial.println(F("getsig         getsig (r)                    getsig 0      "
                   "             Get signature of record (r)"));
  Serial.println(F("sigtrain       sigtrain (r) (sig)            sigtrain 0 "
                   "ZERO            Train one record(r) with signature(sig)"));
  Serial.println(F("settings       settings                      settings      "
                   "             Check current system settings"));
  Serial.println(F("help           help                          help          "
                   "             print this message"));
}
